...
// 行为依据：README 关键事实（bilibili-API-collect 文档 + Bili23-Downloader-main 源码实证 + 本机 curl 实测）
...
async function biliFetch(url, session, init = {}) {
  ...
  try {
    ...
    json = {
      code: -1,
      message: `HTTP ${res.status} 非 JSON 响应（${res.headers.get("content-type") || "无类型"}）${preview ? `: ${preview}` : "（空正文）"}`,
    };
  }
  return { json, res };
}
...
function genBLsid() {
  return `${randomUpperHex(8)}_${Math.floor(Date.now() / 1000)
    .toString(16)
    .toUpperCase()}`;
}
...
async function fetchFingerSpi() {
  ...
  if (json.code !== 0 || !json.data?.b_3)
    throw new Error(`finger/spi 失败: ${json.message || json.code}`);
...
async function fetchBiliTicket(risk) {
  ...
  const hexsign = [...new Uint8Array(sig)]
  ...
  if (json.code !== 0 || !json.data?.ticket)
    throw new Error(`GenWebTicket 失败: ${json.message || json.code}`);
  return {
    ticket: json.data.ticket,
    ttl: json.data.ttl || 259200,
    nav: json.data.nav || null,
  };
}
...
function buildExPayload(userAgent, uuid) {
  const payload = {
    ...
    "3c43": {
      ...
      "6bc5":
        "Google Inc. (NVIDIA)~ANGLE (NVIDIA, NVIDIA GeForce RTX 4060 Laptop GPU (0x000028E0) Direct3D11 vs_5_0 ps_5_0, D3D11)",
      ed31: 0,
      "72bd": 0,
      "097b": 0,
      "52cd": [0, 0, 0],
...
export async function qrcodePoll(qrcodeKey, session) {
  ...
  if (json.code !== 0)
    throw new Error(`轮询失败: ${json.message || json.code}`);
...
export async function nav(session) {
  ...
  if (json.code !== 0 && json.code !== -101)
    throw new Error(`nav 失败: ${json.message || json.code}`);
...
export async function viewInfo(bvid, session) {
  ...
  if (json.code !== 0)
    throw new Error(`获取视频信息失败: ${json.message || json.code}`);
  return json.data;
}
      ...
      return {
        ...
        dash: d.dash
          ? {
              ...
              dolby: (d.dash.dolby?.audio || []).map((t) =>
                mapTrack(t, "dolby"),
              ),
              flac: d.dash.flac
                ? mapTrack(d.dash.flac.audio || d.dash.flac, "flac")
                : null,
            }
          : null,
        durl: d.durl
